...
public sealed class ExtUITabstrip : UITabstrip
{
	...
	public static ExtUITabstrip Create(UIHelper helper)
	{
		...
		var orgOptsContainerWidth = optionsContainer.height;
		var orgOptsContainerHeight = optionsContainer.width;

		var paddingRight = 10; //Options container is Scrollable panel itself(reserves space for scroll - which we don't use)
		optionsContainer.size = new Vector2(orgOptsContainerWidth + paddingRight, orgOptsContainerHeight);

		var tabStrip = optionsContainer.AddUIComponent<ExtUITabstrip>();
