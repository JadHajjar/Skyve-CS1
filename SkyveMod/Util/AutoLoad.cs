...
public class AutoLoad : MonoBehaviour
{
	...
	void Do()
	{
		...
		try
		{
			ParseCommandLine("game|newGame", out var newGame);
			ParseCommandLine("save|loadSave", out var loadSave);
			var loadAsset = ParseCommandLine("editor|loadAsset", out _);
			var newAsset = ParseCommandLine("newAsset", out _);
			var lsm = ParseCommandLine("LSM", out _);
			var lht = ParseCommandLine("LHT", out _);
	...
	static bool IsCrp(string str)
	{
		return str.ToLower().EndsWith(".crp");
	}
	...
	static void UpdateTheme(string mapThemeRef, SimulationMetaData ngs)
	{
		...
		{
			var asset = PackageManager.FindAssetByName(mapThemeRef, UserAssetType.MapThemeMetaData);
			if (asset != null)
			{
	...
	{
		return FindAssetByShortName(name, UserAssetType.SaveGameMetaData);
	}

	static Package.Asset GetMapByName(string name)
	{
		return FindAssetByShortName(name, UserAssetType.MapMetaData);
	}

	static Package.Asset FindAssetByShortName(string name, Package.AssetType assetType)
	{
		return FilterAssets(assetType).FirstOrDefault(a => string.Equals(a.name, name, StringComparison.OrdinalIgnoreCase));
	}

	static Package.Asset GetAMap()
	{
		return Maps.MinBy(m => m.Instantiate<MapMetaData>()?.mapName);
	}
	...
	internal static Package.Asset GetDefaultTheme()
	{
		try
		{
			var name = "BuiltinTerrainMap-Sunny";
	...
	{
		return PackageManager.FilterAssets(new[] { type })
