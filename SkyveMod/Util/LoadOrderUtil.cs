...
internal static class LoadOrderUtil
{
	...
	public static void ResetIsEnabledForAssets()
	{
		...
		{
			var enabledDefault = PackageManager.GetEnabledDefault(asset);
			if (enabledDefault)
			{
				ResetEnabled(asset);
				...
				{
					foreach (var sideAsset in asset.package.FilterAssets(new[] { UserAssetType.CustomAssetMetaData }))
					{
						var customAssetMetaData2 = sideAsset.Instantiate<CustomAssetMetaData>();
	...
	{
		var savedBool = new SavedBool(asset.checksum + ".enabled", PackageManager.assetStateSettingsFile);
		savedBool.Delete();
	}
}
