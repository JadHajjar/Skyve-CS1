...
internal static class StringExtensions
{
	...
	{
		var leftPadding = ((totalLength - stringToCenter.Length) / 2) + stringToCenter.Length;
	...
	{
		if (list is Package p)
		{
			return p.ToString(); // don't print all assets
		}
	...
	internal static string ToSTR<T>(this IEnumerable list, string format)
	{
		...
		var mToString = typeof(T).GetMethod("ToString", new[] { typeof(string) })
