...
using static SkyveMod.Util.LSMUtil;
...
public static class SceneLoadedPatch
{
	...
	static readonly MethodInfo mLogSceneEnded = typeof(SceneLoadedPatch).GetMethod(nameof(LogSceneEnded), true);
	static readonly MethodInfo mEndLoading = typeof(LoadingProfiler).GetMethod(nameof(LoadingProfiler.EndLoading));
	...
	public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, MethodBase original)
	{
		try
		{
			...
			var index = codes.FindLastIndex(c => c.opcode == OpCodes.Ldfld && (c.operand as FieldInfo).Name == "uiScene");
