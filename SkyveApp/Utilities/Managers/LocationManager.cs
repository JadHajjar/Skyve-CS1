...
internal static class LocationManager
{
	...
	public static string SkyveAppDataPath => Combine(AppDataPath, "Skyve");
	public static string SkyveProfilesAppDataPath => Combine(SkyveAppDataPath, "Profiles");
	...
	internal static void RunFirstTimeSetup()
	{
		...
		finally
		{
			...
			if (File.Exists(Combine(AppDataPath, "LoadOrder", "LoadOrderConfig.xml")) && !File.Exists(Combine(SkyveAppDataPath, "SkyveConfig.xml")))
			{
				ExtensionClass.CopyFile(Combine(AppDataPath, "LoadOrder", "LoadOrderConfig.xml"), Combine(SkyveAppDataPath, "SkyveConfig.xml"), true);
			}
		}
	}
