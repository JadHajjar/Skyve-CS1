...
partial class ItemListControl<T>
{
	protected override void OnPaintItemList(ItemPaintEventArgs<T, Rectangles> e)
	{
		...
		if (date.HasValue && !IsPackagePage)
		{
			...
			rects.DateRect = e.DrawLabel(dateText, IconManager.GetSmallIcon("I_UpdateTime"), FormDesign.Design.AccentColor.MergeColor(FormDesign.Design.BackColor, 50), labelRect, large ? ContentAlignment.TopLeft : ContentAlignment.BottomLeft, mousePosition: CursorLocation);
		...
		foreach (var item in e.Item.GetTags(IsPackagePage))
		{
			...
			var tagRect = e.DrawLabel(item.Value, tagIcon, FormDesign.Design.ButtonColor, labelRect, ContentAlignment.BottomLeft, mousePosition: CursorLocation);
		...
		if (compatibilityReport is not null && compatibilityReport.GetNotification() > NotificationType.Info)
		{
			...
			rects.CompatibilityRect = e.DrawLabel(LocaleCR.Get($"{compatibilityReport.GetNotification()}"), IconManager.GetSmallIcon("I_CompatibilityReport"), labelColor, rects.CompatibilityRect, large ? ContentAlignment.MiddleRight : ContentAlignment.BottomRight, mousePosition: CursorLocation);
	...
	private void DrawAuthorAndSteamId(ItemPaintEventArgs<T, Rectangles> e, bool large, Rectangles rects, ILocalPackageWithContents? package, IWorkshopInfo? workshopInfo)
	{
		...
		{
			rects.SteamIdRect = e.DrawLabel(Path.GetFileName(package?.Folder), IconManager.GetSmallIcon("I_Folder"), FormDesign.Design.ActiveColor.MergeColor(FormDesign.Design.ButtonColor, 30), rects.SteamIdRect, large ? ContentAlignment.MiddleLeft : ContentAlignment.BottomLeft, mousePosition: CursorLocation);
		...
		{
			rects.AuthorRect = e.DrawLabel(workshopInfo.Author?.Name, IconManager.GetSmallIcon("I_Developer"), FormDesign.Design.ActiveColor.MergeColor(FormDesign.Design.ButtonColor, 30), rects.AuthorRect, ContentAlignment.TopLeft, mousePosition: CursorLocation);
		}

		rects.SteamIdRect = e.DrawLabel(e.Item.Id.ToString(), IconManager.GetSmallIcon("I_Steam"), FormDesign.Design.ActiveColor.MergeColor(FormDesign.Design.ButtonColor, 30), rects.SteamIdRect, large ? ContentAlignment.MiddleRight : ContentAlignment.BottomLeft, mousePosition: CursorLocation);
	}
