...
public class RequestDataUtil
{
	...
	public static async void Start(List<Domain.Package> packages)
	{
		try
		{
			...
			if (_requestData?.UserId != null)
			{
				...
				_requestData.TotalIncludedAssetSizes = packages.Sum(x => x.Assets?.Sum(x => x.IsIncluded ? x.LocalSize : default) ?? 0) / 8 / 1024;
				_requestData.TotalModSizes = packages.Where(x => x.IsMod).Sum(x => x.FileSize) / 8 / 1024;
				_requestData.TotalAssetSizes = packages.Sum(x => x.Assets?.Sum(x => x.LocalSize) ?? 0) / 8 / 1024;
	...
	public static void Log(string url, long contentLength)
	{
		try
		{
			if (_requestData?.UserId == null)
