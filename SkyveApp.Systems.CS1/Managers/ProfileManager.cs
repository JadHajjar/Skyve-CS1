...
public class ProfileManager : IPlaysetManager
{
    ...
    public void GatherInformation(Playset? profile)
    {
        ...
        profile.Assets = _contentManager.Assets.Where(x => x.IsIncluded).Select(x => new Playset.Asset(x)).ToList();
        profile.Mods = _contentManager.Mods.Where(x => x.IsIncluded).Select(x => new Playset.Mod(x)).ToList();
