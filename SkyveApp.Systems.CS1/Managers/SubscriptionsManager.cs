...
internal class SubscriptionsManager : ISubscriptionsManager
{
    ...
    public SubscriptionsManager(IContentManager contentManager, ILocationManager locationManager, ICitiesManager citiesManager, ISettings settings, ILogger logger, INotifier notifier)
    {
    ...
    {
        PendingSubscribingTo.RemoveAll(x => _contentManager.GetPackageById(x) is not null);
        PendingUnsubscribingFrom.RemoveAll(x => _contentManager.GetPackageById(x) is null);
    }
