...
internal class DlcList : SlickStackedListControl<SteamDlc>
{
	...
	protected override void OnPaintItem(ItemPaintEventArgs<SteamDlc> e)
	{
		...
		e.Graphics.DrawString(e.Item.Description, UI.Font(large ? 9.75F : 7F), new SolidBrush(Color.FromArgb(150, e.HoverState.HasFlag(HoverState.Pressed) ? FormDesign.Design.ActiveForeColor : ForeColor)), textRect.Pad(0, (int)e.Graphics.Measure(e.Item.Name, UI.Font(large ? 11.25F : 9F, FontStyle.Bold)).Height, 0, 0), new StringFormat { Trimming = StringTrimming.EllipsisCharacter, LineAlignment = StringAlignment.Far });

		if (e.Item.ReleaseDate != DateTime.MinValue)
		{
			DrawLabel(e, CentralManager.SessionSettings.ShowDatesRelatively ? e.Item.ReleaseDate.ToLocalTime().ToRelatedString(true, false) : e.Item.ReleaseDate.ToString("D"), Properties.Resources.I_UpdateTime, FormDesign.Design.AccentColor.MergeColor(FormDesign.Design.BackColor, 50), rects.TextRect, ContentAlignment.TopRight);
		}
