namespace SkyveIPatch {
    ...
    internal static class LogExtension {
        public static void Log(this Exception ex) => SkyveIPatch.Log.Exception(ex);
    }

    internal static class Log {
        const string FILENAME = "SkyveIPatch.log";
        ...
        public static void Info(string text) {
            iLogger_.Info("[SkyveIPatch] " + text);
            LogImpl("Info", text);
        }
        public static void Warning(string text) {
            iLogger_.Info("[Warning] [SkyveIPatch] " + text);
            LogImpl("Warning", text);
        }

        public static void Error(string text) {
            iLogger_.Error("[SkyveIPatch] " + text);
            LogImpl("Error", text + "\n" + Environment.StackTrace);
        }
        public static void Exception(this Exception ex) {
            iLogger_.Error("[Exception] [SkyveIPatch] " + ex.Message);
